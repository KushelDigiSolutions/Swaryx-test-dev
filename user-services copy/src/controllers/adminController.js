...
export const getAdminDashboard = async (req, res) => {
  try {
    const data = await getAdminDashboardService(req.user.id);
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: "Error", error: err.message });
  }
...
export const createUser = async (req, res) => {
  try {
    const user = await createUserService(req.user.id, req.body);
...
export const getMyUsers = async (req, res) => {
  try {
    const data = await getMyUsersService(req.user.id, req.query);
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: "Error", error: err.message });
  }
...
export const softDeleteUser = async (req, res) => {
  try {
    await softDeleteUserService(req.user.id, req.params.userId);
    res.json({ message: "User soft delete ho gaya" });
  } catch (err) {
...
export const hardDeleteUser = async (req, res) => {
  try {
    await hardDeleteUserService(req.user.id, req.params.userId);
    res.json({ message: "User permanently delete ho gaya" });
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
